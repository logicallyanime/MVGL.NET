...
internal static class Mdb1Format
{
    ...
    internal static void CryptArray(Span<byte> array, long offset)
    {
...
internal sealed class XorStream : Stream
{
    ...
    private readonly long _destOffset;   // emulated abs offset
    private readonly long _sourceOffset; // original abs offset
    private readonly bool _isCrypted;

    public XorStream(Stream baseStream, long destOffset, long sourceOffset = -1)
    {
        _base =  baseStream;
    ...
    {
        var posBefore = _base.Position;
        var n = _base.Read(buffer, offset, count);
        if(n <= 0) return n;
        
        var span = buffer.AsSpan(offset, n);
        
        // Decrypt Original
        if(_sourceOffset >= 0)
            Mdb1Format.CryptArray(span, _sourceOffset + posBefore);
        
        // Re-encrypt at new offset
        Mdb1Format.CryptArray(span, _destOffset + posBefore);
        return n;
    }
